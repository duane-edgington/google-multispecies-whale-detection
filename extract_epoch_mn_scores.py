...
def extract_mn_class_score(json_data):
    """
    Extract the Mn class score from the JSON data by finding the index of "Mn"
    ...
    if 'class_names' in json_data and 'scores' in json_data:
        ...
        if (isinstance(class_names, list) and isinstance(scores, list) and 
            ...
            # Find the index of "Mn" in class_names
            try:
                mn_index = class_names.index("Mn")
                return float(scores[mn_index])
            except ValueError:
                # "Mn" not found in class_names
                pass
            except IndexError:
    ...
    # If no Mn score found, return 0.0
    return 0.0

def process_json_directory(input_directory, output_file=None):
    """
    Process JSON files and create CSV with epoch seconds and Mn class score.
    """
    ...
    if output_file is None:
        ...
        output_file = dir_path.parent / f"{base_name}_epoch_mn_scores.csv"
    else:
    ...
    for json_file in json_files:
        try:
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                mn_score = extract_mn_class_score(data)
                
                results.append({
                    'epoch_seconds': epoch_seconds,
                    'Mn_class_score': mn_score,
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['epoch_seconds', 'Mn_class_score']
        ...
        for result in results:
            writer.writerow({
                ...
                'Mn_class_score': result['Mn_class_score']
            })
    ...
    print(f"Mn score range: {min(r['Mn_class_score'] for r in results):.6f} to {max(r['Mn_class_score'] for r in results):.6f}")
    
    # Count files with non-zero Oo scores
    non_zero_count = sum(1 for r in results if r['Mn_class_score'] > 0)
    print(f"Files with Mn scores > 0: {non_zero_count}/{len(results)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract epoch seconds and Mn class scores from JSON files. Mn score is extracted from the score list at the position where class_names contains "Mn".',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s MARS_20180413_065913_resampled_24kHz
  %(prog)s /path/to/data --output mn_scores.csv
  %(prog)s /path/to/json_files -o results/epoch_mn_scores.csv
        """
    )
