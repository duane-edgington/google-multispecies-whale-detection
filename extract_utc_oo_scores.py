...
def calculate_utc_datetime(base_datetime, chunk_number):
    """
    Calculate UTC datetime from base datetime and chunk number.
    Each chunk represents 5 seconds, so offset = (chunk_number - 1) * 5 seconds
    """
    time_offset = timedelta(seconds=(chunk_number - 1) * 5)  # 5 seconds per chunk
    utc_datetime = base_datetime + time_offset
    return utc_datetime

def format_utc_datetime(dt):
    """
    Format UTC datetime in ISO 8601 format with timezone.
    Example: 2018-04-13T06:59:13Z
    """
...
def extract_oo_class_score(json_data):
    ...
    in the "class_names" list and getting the corresponding score from the "scores" list.
    """
    # Look for class_names and scores lists
    if not isinstance(json_data, dict):
        return 0.0
    
    # Check if we have both class_names and scores lists
    ...
    # Also check for singular 'score' as fallback (in case some files use different key)
    if 'class_names' in json_data and 'score' in json_data:
        class_names = json_data['class_names']
...
def process_json_directory(input_directory, output_file=None):
    """
    Process JSON files and create CSV with UTC datetime and Oo class score.
    """
    ...
    if output_file is None:
        ...
        output_file = dir_path.parent / f"{base_name}_UTC_oo_scores.csv"
    else:
    ...
    for json_file in json_files:
        try:
            with open(json_file, 'r', encoding='utf-8') as f:
                ...
                utc_datetime = calculate_utc_datetime(base_datetime, chunk_number)
                formatted_utc = format_utc_datetime(utc_datetime)
                oo_score = extract_oo_class_score(data)
                
                results.append({
                    'utc_datetime': formatted_utc,
                    'Oo_class_score': oo_score,
                    'filename': json_file.name,
                    'chunk_number': chunk_number,
                    'raw_datetime': utc_datetime  # Keep for sorting/range calculation
                })
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['utc_datetime', 'Oo_class_score']
    ...
    print(f"Time range: {results[0]['utc_datetime']} to {results[-1]['utc_datetime']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Extract UTC datetime and Oo class scores from JSON files. Oo score is extracted from the scores list at the position where class_names contains "Oo".',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s MARS_20180413_065913_resampled_24kHz
  %(prog)s /path/to/data --output utc_scores.csv
  %(prog)s /path/to/json_files -o results/UTC_oo_scores.csv
        """
    )
    ...
    parser.add_argument(
        '-o', '--output',
        dest='output_file',
        help='Output CSV filename (optional). If not provided, will be automatically derived from input directory name with _UTC_oo_scores.csv suffix.'
    )
    ...
    try:
        if args.debug:
            ...
            if json_files:
                ...
                # Also check what keys are available
                print(f"\nAvailable keys in JSON: {list(data.keys())}")
                if 'class_names' in data:
                    print(f"class_names: {data['class_names']}")
                if 'scores' in data:
